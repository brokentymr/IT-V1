...
export interface ShortFormBuilder { build(input: { substance: Substance; newsletterMarkdown: string; voice: string }): Promise<ShortFormPack> }

const DEFAULT_DISCLOSURE = "Education, not advice.";

export class ClaudeShortFormBuilder implements ShortFormBuilder {
  async build(input: { substance: Substance; newsletterMarkdown: string; voice: string }): Promise<ShortFormPack> {
...
export async function buildShortForm(substance: Substance, newsletter: Newsletter, builder: ShortFormBuilder = new ClaudeShortFormBuilder()): Promise<ShortFormPack> {
  const pack = await builder.build({ substance, newsletterMarkdown: newsletter.markdown, voice: HOUSE_VOICE });
